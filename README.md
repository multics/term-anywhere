...
Resize mode is available only for a tmux session opened through the app; ordinary terminals keep their normal double-tap behavior. To resize a tmux pane, double-tap the terminal, then drag near its border with one finger. Resize mode prepares the selected tmux session’s layout, then snaps the press to the nearest border. A loading indicator shows when pane borders are being read. Tap Done resizing or double-tap again to restore scrolling. You can also drag with two fingers outside resize mode, with their midpoint on the border. This requires tmux mouse mode and its border-drag binding. The app does not change server settings. Local text selection blocks remote dragging; clear the selection first.
