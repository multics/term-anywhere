...
| 9 | iPhone landscape | Request landscape after connection; leave iPad orientation under user control | Implemented and tested: 10 Simulator terminal tests pass, including landscape and restoration after disconnect. Physical iPad confirms no forced rotation; rendered layouts inspected. Signed build installed on both devices. Physical iPhone rotation check is pending because the phone locked |
